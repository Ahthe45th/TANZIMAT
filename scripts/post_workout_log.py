...
try:
    ...
    print("Error: post_workout_log_questions.json not found. Please create it with your questions.")
    questions = []
except json.JSONDecodeError:
    print("Error: Could not decode post_workout_log_questions.json. Ensure it's valid JSON.")
    questions = []
...
try:
    ...
    message = f"Post workout log saved to {file_path}"
    print(message)
    subprocess.run(["notify-send", "Post Workout Log", message])
except IOError as e:
    error_message = f"Error writing to file {file_path}: {e}"
    print(error_message)
    subprocess.run(["notify-send", "Post Workout Log Error", error_message])
except Exception as e:
