...
try:
    ...
    print("Error: weekly_review_questions.json not found. Please create it with your questions.")
    questions = []
except json.JSONDecodeError:
    print("Error: Could not decode weekly_review_questions.json. Ensure it's valid JSON.")
    questions = []
...
try:
    ...
    message = f"Weekly review saved to {file_path}"
    print(message)
    subprocess.run(["notify-send", "Weekly Review", message])
except IOError as e:
    error_message = f"Error writing to file {file_path}: {e}"
    print(error_message)
    subprocess.run(["notify-send", "Weekly Review Error", error_message])
except Exception as e:
