...
QUESTIONS_FILE = os.path.join(SCRIPT_DIR, 'daily_reflection_questions.json')
...
try:
    ...
    print("Error: daily_reflection_questions.json not found. Please create it with your questions.")
    questions = []
except json.JSONDecodeError:
    print("Error: Could not decode daily_reflection_questions.json. Ensure it's valid JSON.")
    questions = []
...
try:
    ...
    message = f"Daily reflection saved to {file_path}"
    print(message)
    subprocess.run(["notify-send", "Daily Reflection", message])
except IOError as e:
    error_message = f"Error writing to file {file_path}: {e}"
    print(error_message)
    subprocess.run(["notify-send", "Daily Reflection Error", error_message])
except Exception as e:
