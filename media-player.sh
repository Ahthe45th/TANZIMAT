...
ENTERTAINMENT_DIRdos="/mnt"
# Find all media files in the specified directories
# You can add more extensions here if needed (e.g., .avi, .mov)
MEDIA_FILES=$(find "$WATCHLIST_DIR" "$ENTERTAINMENT_DIR" "$ENTERTAINMENT_DIRdos" -type f \( -iname "*.mp4" -o -iname "*.mkv" -o -iname "*.webm" -o -iname "*.mp3" -o -iname "*.flac" -o -iname "*.ogg" \))
